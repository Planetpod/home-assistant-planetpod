...
> Requires **pod firmware 1.1.11 or later**. Currently supports **one pod per Home Assistant instance**.
...
- **Local mode:** the pod reachable on the same network as Home Assistant, and Home Assistant's HTTP integration enabled (on by default). Currently supports **one pod per Home Assistant instance**.
